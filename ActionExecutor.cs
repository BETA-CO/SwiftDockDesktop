...
namespace SwiftDock
{
    public static class ActionExecutor
    {
        ...
        private static void RunCommand(string command)
        {
            ...
            // Run clipboard and typing simulation in an STA thread because Clipboard access requires STA
            var thread = new System.Threading.Thread(() =>
            {
                try
                {
                    ...
                    // 1. Detect active IDE and open/focus its integrated terminal
                    if (activeTitle.Contains("visual studio code") || activeTitle.Contains("vscode") || 
                        activeTitle.Contains(" - code - ") || activeTitle.Contains("antigravity"))
                    {
                        // VS Code / Antigravity: Ctrl + Shift + ` (VK_CONTROL = 0x11, VK_SHIFT = 0x10, VK_OEM_3 = 0xC0)
                        SimulateKeyCombo(new byte[] { 0x11, 0x10, 0xC0 });
                        inIde = true;
                        System.Threading.Thread.Sleep(500); // Wait for terminal to open and initialize
                    }
                    else if (activeTitle.Contains("android studio") || activeTitle.Contains("intellij") || 
                             activeTitle.Contains("pycharm") || activeTitle.Contains("webstorm") || 
                             activeTitle.Contains("clion") || activeTitle.Contains("rider") || 
                             activeTitle.Contains("datagrip") || activeTitle.Contains("phpstorm"))
                    {
                        // JetBrains IDEs: Alt + F12 (VK_MENU = 0x12, VK_F12 = 0x7B)
                        SimulateKeyCombo(new byte[] { 0x12, 0x7B });
                        inIde = true;
                        System.Threading.Thread.Sleep(500); // Wait for terminal to open and take focus
                    }
                    else if (activeTitle.Contains("visual studio") && !activeTitle.Contains("code"))
                    {
                        // Visual Studio: Ctrl + ` (VK_CONTROL = 0x11, VK_OEM_3 = 0xC0)
                        SimulateKeyCombo(new byte[] { 0x11, 0xC0 });
                        inIde = true;
                        System.Threading.Thread.Sleep(500); // Wait for terminal to open and take focus
                    }

                    // 2. If not in an IDE, launch standard CMD window
                    if (!inIde)
                    {
                        try
                        {
                            ...
                            Process.Start(psi);
                            System.Threading.Thread.Sleep(600); // Wait for cmd.exe to launch and take focus
                        }
                        catch (Exception ex)
                        {
                            Debug.WriteLine($"Failed to launch cmd.exe: {ex.Message}");
                        }
                    }

                    // 3. Backup original clipboard text
                    ...
                    {
                        Debug.WriteLine($"Failed to backup clipboard: {ex.Message}");
                    }

                    // Set command text to clipboard
                    try
                    {
                    ...
                    {
                        Debug.WriteLine($"Failed to set command to clipboard: {ex.Message}");
                        return;
                    }

                    // Wait for clipboard to register
                    System.Threading.Thread.Sleep(50);

                    // 4. Simulate Ctrl + V
                    const byte VK_CONTROL = 0x11;
                    const byte V_KEY = 0x56;
                    const byte VK_RETURN = 0x0D;

                    // Press Ctrl
                    keybd_event(VK_CONTROL, 0, 0, UIntPtr.Zero);
                    // Press V
                    keybd_event(V_KEY, 0, 0, UIntPtr.Zero);
                    // Release V
                    keybd_event(V_KEY, 0, KEYEVENTF_KEYUP, UIntPtr.Zero);
                    // Release Ctrl
                    ...
                    // Wait before restoring clipboard to prevent race condition
                    System.Threading.Thread.Sleep(100);

                    // 6. Restore original clipboard
                    try
                    {
                        if (hasOriginalText && originalText != null)
                        {
                    ...
                    {
                        Debug.WriteLine($"Failed to restore clipboard: {ex.Message}");
                    }
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"Simulation thread error: {ex.Message}");
                }
            });
